# Script to autogenerate ad upload template from csv file. 

# modules

import csv

# initialize an empty list and assign to csv_dict
csv_dict = []

# break this function into separate functions, but maybe wait set up classes first..??? 
# read csv file as dictionary and append new dictionary to csv_dict 
# sets keys in csv_dict that match the headers in ad upload template
def read_csv_to_dictionary(csv_file):
        ...
        for row in read_csv_dict:
            ...
            if 'JavaScript Tag' in row:
                csv_dict.append({
                    ...
                    'Ad Serving': 'DCM',
                    'Ad Domains': '',
                    'Script Type': 'dcm-ins-2',
                    'Script Tag': row['JavaScript Tag'],
                    'Third Party IDs': row['Placement ID']
                })
            ...
            # DV wrapped DCM display tags
                ...
                csv_dict.append({
                    ...
                    'Ad Domains': '',
                    'Script Type': 'dcm-ins-2',
                    'Script Tag': row['Blocking Javascript Tag'],
                    'Third Party IDs': row['Placement ID']
                })
            ...
            # standard Sizmek display tags
                ...
                csv_dict.append({
                    ...
                    'Ad Serving': 'Sizmek',
                    'Ad Domains': '',
                    'Script Type': 'sizmek_default',
                    'Script Tag': row['Script | SECURED'],
                    'Third Party IDs': row['PLACEMENT ID']  # use excel TRIM() func to remove whitespace around 'PLACEMENT ID' in csv file
                })
            ...
            # DV wrapped Sizmek display tags
                ...
                csv_dict.append({
                    ...
                    'Ad Domains': '',
                    'Script Type': 'sizmek_default',
                    'Script Tag': row['Display Blocking Script | SECURED'],
                    'Third Party IDs': row['Placement ID']
                })
            ...
            # standard DCM tracking tags
                ...
                csv_dict.append({
                    ...
                    'Ad Unit': '',
                    'Ad Dimension': '',
                    'Creative Name': row['Creative File Name'],
                    'Ad Serving': 'Cluep',
                    'Click Tracker URLs': row['Click Tag'],
                    'Impression Tracker URLs': row['Impression Tag (image)'],
                    'Ad Domains': '',
                    'Script Type': '',
                    'Script Tag': '',
                    'Append HTML': '',
...
# search for and remove GDPR macros in DCM JS tags
def remove_gdpr_macro_dcm_js_tag():
    for row in csv_dict:
    ...
    data-dcm-addtl-consent='addtl_consent=${ADDTL_CONSENT}'""" in row['Script Tag']:
            tag_minus_gdpr = row['Script Tag'].replace("""
    data-dcm-gdpr-applies='gdpr=${GDPR}'
    data-dcm-gdpr-consent='gdpr_consent=${GDPR_CONSENT_755}'
...
# search for and move IAS tags from 'Impression Tracker URLs' to 'Append HTML'
def move_ias_trackers():
    for row in csv_dict:
...
# search for and remove GDPR macros in DCM imp trackers
def remove_gdpr_macro_dcm_imp_tag():
    for row in csv_dict:
...
# search for and remove <img> tag wrapper around DCM imp trackers
def remove_img_wrapper_dcm_imp_tag():
    for row in csv_dict:
...
# dictionary of ad types and corresponding sub-strings in tags
ad_type_dict = {
...
def assign_ad_type(dict_key='Script Tag'):
    for ad_type in ad_type_dict:
        for dimensions in ad_type_dict[ad_type]:
            for row in csv_dict:
...
def assign_ad_unit(dict_key='Script Tag'):
    for ad_unit in ad_unit_dict:
        for dimensions in ad_unit_dict[ad_unit]:
            for row in csv_dict:
                if dimensions in row[dict_key]:
                    row['Ad Unit'] = ad_unit


# dictionary of ad units and corresponding sub-strings in tags
ad_dimensions_dict = {
...
def assign_ad_dimensions(dict_key='Script Tag'):
    for ad_dimensions in ad_dimensions_dict:
        for dimensions in ad_dimensions_dict[ad_dimensions]:
            for row in csv_dict:
                if dimensions in row[dict_key]:
...
def assign_creative_name():
    for row in csv_dict:
        if row['Creative Name'] != '':
            ...
            creative_list = []
            creative_list = input_creative_names()
...
# input domain name and sets 'Ad Domains' based on input
def ad_domain():
    ad_domain = input("Enter ad domain: ")
    for row in csv_dict:
        row['Ad Domains'] = ad_domain


# identify column headers for new csv file
...
def create_new_csv():  
    with open('test_ad_upload.csv', 'w', newline='') as new_csv:  # include newline='' prevents empty rows after each dict is written        
        ...
        new_csv_writer = csv.DictWriter(new_csv, fieldnames=header_names)
...
read_csv_to_dictionary('real_dcm_tags_test.csv')
remove_gdpr_macro_dcm_js_tag()
move_ias_trackers()
remove_gdpr_macro_dcm_imp_tag()
remove_img_wrapper_dcm_imp_tag()
assign_ad_type('Ad Name')
assign_ad_unit('Ad Name')
assign_ad_dimensions('Ad Name')
assign_creative_name()
ad_domain()
create_new_csv()
